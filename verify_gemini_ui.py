...
def drive(label: str) -> None:
    ...
    at.run()
    # Task 4.3: the duplicate per-dock Investigation buttons were removed;
    # its "Open Detail View" button (key open_detail_{fleet_id}) is now
    # the single entry point into the Tri-View panel.
    detail_btn = at.button(key=f"open_detail_{fleet.id}")
    assert detail_btn is not None, f"[{label}] no Open Detail View button"
    detail_btn.click()
    at.run()
